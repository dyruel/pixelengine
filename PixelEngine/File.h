...
public:
	...
	~FileSystem() {
		...
		if (!mz_zip_reader_end(&m_pak)) {
			ILogger::log("FileSystem :: Error while closing FileSystem.\n");
		}
	};

	bool mount(const char * pakname) {
		memset(&m_pak, 0, sizeof(m_pak));
		mz_bool r = mz_zip_reader_init_file(&m_pak, pakname, 0);
		if (!r) {
			ILogger::log("FileSystem :: Error while loading the pak file %s\n", pakname);
			return false;
		}
	...
	*/
	...
	bool open(const char *filename) {
		...
		m_file.data = mz_zip_reader_extract_file_to_heap(&m_pak, filename, &m_file.size, 0);
		if (!m_file.data) {
	...
	bool isOpened() {
		return m_file.data != NULL;
	}

	bool read(void* buffer, const u32& size) {
		if (!isOpened()) {
			ILogger::log("FileSystem :: Attempting to read a non opened file.\n");
			return false;
		}

		if (m_file.size < m_file.offset) {
			ILogger::log("FileSystem :: Reading beyond the file.\n");
			return false;
		}

		void * r = memcpy(buffer, ((u8*) m_file.data) + m_file.offset, (size_t)size);

		return true;
	}

	bool seek(const u32& offset) {
		if (!isOpened()) {
			ILogger::log("FileSystem :: Attempting to seek a non opened file.\n");
			return false;
		}

		m_file.offset = offset;
	}
